...
export interface KeySplitKeyboardProps {
...
export default function KeySplitKeyboard({ onZonesChange }: KeySplitKeyboardProps = {}) {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (drag.type === 'handle') {
        ...
        if (drag.handle === 'left') {
          ...
          // Hard wall against previous zone
        ...
        // Update pure visual representation of the ghost
        setDragState(prev => prev ? { ...prev, mouseX: e.clientX } : null);

        // Find intersections dynamically
        ...
        if (hoveredZone) {
          ...
          if (doSwap) {
            ...
            // Adjust visual start coordinate exactly by the physical leap to prevent snap-back artifact
            ...
            // Swap their logical boundaries natively
            applyZones(currentZones.map(z => {
  ...
  const handleKeyEnter = (e: React.MouseEvent, note: number) => {
    if (e.buttons === 1) { // Left-click drag playing
      playNote(note);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="font-semibold text-[14px] text-gray-700 select-none">Input</span>
      </div>
      ...
      {!isCollapsed && (
        ...
        {sortedZones.map((zone, idx) => {
          ...
          // Apply horizontal translation visually if ghost dragging
        ...
        {/* Pass 2: Seam Drag Handles (rendered on top of all zones) */}
